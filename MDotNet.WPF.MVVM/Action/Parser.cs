namespace MDotNet.WPF.MVVM.Action
{
	...
	/// <summary>
	///   Parses text into a fully functional set of <see cref = "System.Windows.Interactivity.TriggerBase" /> instances with <see
	///    cref = "ActionMessage" />.
	/// </summary>
	public static class Parser
	{
		private static readonly Regex _sLongFormatRegularExpression =
			new Regex( @"^[\s]*\[[^\]]*\][\s]*=[\s]*\[[^\]]*\][\s]*$" );

		private static readonly IConventionManager _sConventionManager;
		...
		public static Func<DependencyObject, string, TriggerBase> CreateTrigger = ( target, triggerText ) => {
			if ( triggerText == null )
			{
				var defaults = _sConventionManager.GetElementConvention( target.GetType() );
		...
		static Parser()
		{
			_sConventionManager = IoC.GetInstance<IConventionManager>();
		}
		/// <summary>
		...
		public static IEnumerable<TriggerBase> Parse( DependencyObject target, string text )
		{
			...
			{
				var triggerPlusMessage = _sLongFormatRegularExpression.IsMatch( messageText )
				                         	? Split( messageText, '=' )
		                                  ...
		                                  BindingMode bindingMode )
		{
			...
			if ( string.IsNullOrEmpty( path ) )
				path = _sConventionManager.GetElementConvention( element.GetType() ).ParameterProperty;
